...
I = I.reshape(M,M)
dec_func = list()
newI = np.random.rand(M,M) * 1.0e-5
...
X_labeled = np.array(min_max_scaler.fit_transform(X_labeled.tolist()))
...
for line in target_data_file :
    ...
    Y_target.append(arrr[0])
X_target = min_max_scaler.transform(X_target)
...
file_output("/"+time.strftime("%H:%M:%S")+"predictMat",prediclist.tolist())

fileo = open(new_data+"/YwineDT_"+time.strftime("%H:%M:%S"),'w')
